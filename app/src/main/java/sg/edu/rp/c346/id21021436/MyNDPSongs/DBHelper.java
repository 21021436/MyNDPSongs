...
//        db.close();
    ...
    //Selecting distinct year and putting it into ArrayList
    public ArrayList<Integer> distinctYears(){

        ArrayList<Integer> resultYearList = new ArrayList<>();
        SQLiteDatabase db = this.getReadableDatabase();


        //Using rawQuery

        String query = "SELECT DISTINCT " + COLUMN_YEAR + " FROM " + TABLE_SONGS;
        Cursor cursor = db.rawQuery(query, null);
        if (cursor.moveToFirst()){
            do {resultYearList.add(cursor.getInt(0));}
            while (cursor.moveToNext());
        }
    ...
    //Selecting songs by year

    public ArrayList<Song> songsYear(String year){
        ArrayList<Song> yearSongList = new ArrayList<>();
        SQLiteDatabase db = this.getReadableDatabase();
        String[] columns = {COLUMN_ID, COLUMN_SONG_TITLE, COLUMN_SINGER, COLUMN_YEAR, COLUMN_STARS};
        String condition = COLUMN_YEAR + " = ?";
        String[] args = {year};
        Cursor result = db.query(TABLE_SONGS, columns, condition, args, null, null, null, null);
        if (result.moveToFirst()) do{
            int id = result.getInt(0);
            String title = result.getString(1);
            String singer = result.getString(2);
            int songYear = result.getInt(3);
            int stars = result.getInt(4);
            Song newSong = new Song(id, title, singer, songYear, stars);
            yearSongList.add(newSong);
